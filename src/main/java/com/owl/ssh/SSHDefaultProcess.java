...
import com.owl.core.model.Server;
import org.slf4j.*;
...
public class SSHDefaultProcess implements ISSHProcess{
    ...
    private final Pattern patternJar = Pattern.compile("(?<=java -jar\\s)(.*)");
    ...
    public void init(Config config) {
        ...
        this.killOutTime = config.getKillOutTime();
    }
    ...
    public void upload() throws IOException, JSchException, SftpException {
        ...
        try{
            ...
            sftp.connect();
            sftp.put(jarIn, liunxPath, new UploadMonitor(this.localPath, this.liunxPath), ChannelSftp.OVERWRITE);
        }finally {
            if(jarIn != null) {
                jarIn.close();
            }
    ...
    public void shutDownOld() throws Exception {
        ...
        //<3>记录停止的Jar
        String ps = "ps -ef|grep '%s'\n";
        String psPattermStr = "(?<=%s)(.*)(?=\r\n)";
        List<String> list = new ArrayList<String>();
        for(String pid : pids) {
            outstream.write(String.format(ps, pid).getBytes());
            outstream.flush();
            Thread.sleep(1000);
            if(instream.available() >= 0) {
                byte[] psBytes = new byte[instream.available()];
                int readLen = instream.read(psBytes);
                String s = new String(psBytes, 0, readLen, "UTF-8");
                Pattern psPattern = Pattern.compile(String.format(psPattermStr, pid));
                Matcher psMatcher = psPattern.matcher(s);
                while(psMatcher.find()) {
                    Matcher matcherJar = patternJar.matcher(psMatcher.group());
                    while(matcherJar.find()) {
                        list.add(matcherJar.group());
                    }
                }
            }
        }
        if(list.size() != 0) {
            this.oldJarPath = list.get(0);
        }
        if(list.size() > 1) {
            logger.warn("启动进程大于1,JSON:{}", JSON.toJSONString(list));
        }
        //<3>停止旧服务
        long timeOut = (killOutTime / 1000) + 1;
        for(String pid : pids) {
